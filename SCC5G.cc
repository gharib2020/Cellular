...
/**
 *
 * \file SCC5G based on \file cttc-3gpp-channel-nums-fdm.cc of nr package
 * \ingroup examples
 * \brief the full implementation of SCC5G: A PQC-based Architecture for Highly Secure Critical Communication over Cellular Network in Zero-Trust Environment 
 *
 * The example implement the SCC5G handshake under multiple loads of gaming, voice 
 * and video traffic.
...
uint16_t authPort = 88;

void setupmobility(double, double,double,NodeContainer,NodeContainer,uint32_t,int64_t,uint16_t,uint16_t); 
void GnbSetAttribute(uint16_t,NetDeviceContainer);
...
{
public:
   authProcess(uint32_t,uint32_t,double);
   void initAuthPacket();
   void authenticate();
   void acceptDest(Ptr<Socket>,const ns3::Address&);
   void receiveDest (Ptr<Socket>);   
   void verReq();
   void acceptHAKF(Ptr<Socket>,const ns3::Address&);
   void verRes();
   void authRes();
...
authProcess::authProcess(uint32_t src, uint32_t dest,double authTime)
  : source(src),
    destination(dest),
    authSuccess(false),
    startTime(authTime),
...
//##################### IP Setup ##################### 
    ...
    authProcess authenticationInstance(0, 1,authTime);//the node numbering tarts from 0 to ueNum-1
...
void setupmobility(double X, double Y, double Z, NodeContainer ueNodes,NodeContainer gnbNodes,uint32_t mobilityModel,int64_t streamIndex,uint16_t gNbNum,uint16_t ueNum)
{
//setup gNodeBs
...
void GnbSetAttribute(uint16_t gNbNum,NetDeviceContainer enbNetDev)
{
...
{ 
  //std::cout<<"acceptDest, Stage= "<<this->stage<<endl;
...
authProcess::receiveDest (Ptr<Socket> socket)
 {
   ...
   else if(this->stage==3){//Verification response
     socket->SetRecvCallback (MakeCallback (&authProcess::authRes,this));}    
 }
...
authProcess::verReq(){
 ...
 //Create and bind a HAKF socket... 
  HAKFsocket = Socket::CreateSocket (remoteHostContainer.Get (0),    TcpSocketFactory::GetTypeId ());  
  TypeId tid = TypeId::LookupByName ("ns3::TcpNewReno");
  Config::Set ("/NodeList/*/$ns3::TcpL4Protocol/SocketType", TypeIdValue (tid));
  InetSocketAddress local= InetSocketAddress (Ipv4Address::GetAny (), authPort);
  HAKFsocket->Bind(local);
  HAKFsocket->Listen();
  HAKFsocket->SetAcceptCallback (MakeNullCallback<bool, Ptr<Socket>,const Address &> (),MakeCallback(&authProcess::acceptHAKF,this));
  ...
  if(stage==1){//Authentication requested 
    destSocket= Socket::CreateSocket (ueNodes.Get (this->destination),  TcpSocketFactory::GetTypeId ());
  destSocket->Bind ();
  destSocket->Connect (InetSocketAddress (internetIpIfaces.GetAddress (1), authPort)); 
  bool flag=false;//Flag representing the channel unavailability
  while (!flag)
  {
  ...
  if(stage==4){//authentication response has sent
    sourceSocket= Socket::CreateSocket (ueNodes.Get (this->source),  TcpSocketFactory::GetTypeId ());
  sourceSocket->Bind ();
  sourceSocket->Connect (InetSocketAddress (internetIpIfaces.GetAddress (1), authPort)); 
  bool flag=false;//Flag representing the channel unavailability
  while (!flag)
  {
     if(sourceSocket->GetTxAvailable ()>= tcpPacketSizeAuth)
...
void 
authProcess::acceptHAKF(Ptr<Socket> socket,const ns3::Address& from)
{ 
  socket->SetRecvCallback (MakeCallback (&authProcess::verRes,this));
}

void 
authProcess::verRes(){ 
  if(stage==2){//Auth req received by destination 
    destSocket = Socket::CreateSocket (ueNodes.Get (this->destination),    TcpSocketFactory::GetTypeId ());  
  TypeId tid = TypeId::LookupByName ("ns3::TcpNewReno");
  Config::Set ("/NodeList/*/$ns3::TcpL4Protocol/SocketType", TypeIdValue (tid));
  InetSocketAddress local= InetSocketAddress (Ipv4Address::GetAny (), authPort);
  ...
  HAKFsocket= Socket::CreateSocket (remoteHostContainer.Get (0),  TcpSocketFactory::GetTypeId ());
  HAKFsocket->Bind ();
  HAKFsocket->Connect (InetSocketAddress (ueIpIface.GetAddress (this->destination), authPort)); 
  bool flag=false;//Flag representing the channel unavailability
    while (!flag)
    {
       if(HAKFsocket->GetTxAvailable ()>= tcpPacketSizeAuth)
         flag=true;
       int amountSent = HAKFsocket->Send (&DATA[tcpPacketSizeAuth], tcpPacketSizeAuth, 0);
...
//############################################################  
  if(stage==5){//Auth res received by source node 
  sourceSocket = Socket::CreateSocket (ueNodes.Get (this->source),    TcpSocketFactory::GetTypeId ());  
  TypeId tid = TypeId::LookupByName ("ns3::TcpNewReno");
  Config::Set ("/NodeList/*/$ns3::TcpL4Protocol/SocketType", TypeIdValue (tid));
  InetSocketAddress local= InetSocketAddress (Ipv4Address::GetAny (), authPort);
  ...
  HAKFsocket= Socket::CreateSocket (remoteHostContainer.Get (0),  TcpSocketFactory::GetTypeId ());
  HAKFsocket->Bind ();
  HAKFsocket->Connect (InetSocketAddress (ueIpIface.GetAddress (this->source), authPort)); 
  bool flag=false;//Flag representing the channel unavailability
    while (!flag)
    {
       if(HAKFsocket->GetTxAvailable ()>= tcpPacketSizeAuth)
...
authProcess::receiveSource(Ptr<Socket> socket){
  if(this->stage==4)
    socket->SetRecvCallback (MakeCallback (&authProcess::verReq,this));
  else if(this->stage==6){//verifiation response sent to the source node
    this->stage=7;//Authentication process terminated
    this->authSuccess=true;    
    this->HAKFsocket->Close();
    this->sourceSocket->Close();
    if(endTime==0)  
      this->endTime=Simulator::Now ().GetSeconds ();
    cout<<"Authentication time: "<<this->endTime-this->startTime<<endl;
  }
}
